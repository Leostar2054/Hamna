...
class SpatialAttention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.conv1 = Conv2D(filters=self.filters, kernel_size=1, activation='sigmoid', padding='same')
        return self.conv1
...
def model():
    ...
    attention_m = sp.build(input_shape=(64,1024,1))
    x_ver_1_with_attention = attention_m(tf.expand_dims(x_ver_1,axis=-1))
    orig_shape_x_ver = tf.squeeze(x_ver_1_with_attention, axis=-1)


    h_hor_1 = Bidirectional(LSTM(units=256, activation='tanh', return_sequences=True, go_backwards=False))(x_hor_1)
    h_ver_1 = Bidirectional(LSTM(units=256, activation='tanh', return_sequences=True, go_backwards=False))(orig_shape_x_ver)
    ...
    attention_h = sp.build(input_shape=(8, 8, 1))
    H_with_attention_ = attention_h(tf.expand_dims(H_with_attention, axis=-1))
    orig_shape_attention_h = tf.squeeze(H_with_attention_, axis=-1)

        # Decoder Network
        # 8,8,512
    u1 = UpSampling2D(size=(2,2))(orig_shape_attention_h) # 16,16,512
